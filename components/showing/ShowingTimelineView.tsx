...
import {
  ...
  ChevronRight, Phone, ShieldCheck, UserCheck, Flame, User, Filter, Search
} from 'lucide-react';
...
export function ShowingTimelineView({ appointments, onOpenDetail, onClaim }: ShowingTimelineViewProps) {
  ...
  // Filter & sort appointments by search, date ascending, then time ascending
  ...
  return (
    <div className="space-y-4">
      {/* QUICK DATE SELECTOR BAR (Matching LeadsPage layout 100%) */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3.5 shadow-sm space-y-3">
        <div className="flex flex-col gap-3">
          ...
          {/* Search Input & 2 Date Pickers (100% matching LeadTimelineView) */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1 border-t border-slate-100 dark:border-slate-800">
          <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 text-center">
            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold block uppercase">Tổng ca dẫn</span>
            <span className="text-lg font-black text-slate-900 dark:text-white">{stats.total} ca</span>
          </div>
          <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 rounded-xl p-2.5 text-center">
            <span className="text-[10px] text-blue-700 dark:text-blue-300 font-semibold block uppercase">Đang di chuyển</span>
            <span className="text-lg font-black text-blue-800 dark:text-blue-200">{stats.onTheWay} ca</span>
          </div>
          <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 rounded-xl p-2.5 text-center">
            <span className="text-[10px] text-emerald-700 dark:text-emerald-300 font-semibold block uppercase">Đã Check-in</span>
            <span className="text-lg font-black text-emerald-800 dark:text-emerald-200">{stats.checkedIn} ca</span>
          </div>
          <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl p-2.5 text-center">
            <span className="text-[10px] text-amber-700 dark:text-amber-300 font-semibold block uppercase">🎉 Chốt cọc</span>
            <span className="text-lg font-black text-amber-900 dark:text-amber-200">{stats.dealed} ca</span>
          </div>
        </div>
      </div>
      ...
      {timelineItems.length === 0 ? (
        <Card className="border-dashed border-2 border-slate-200 dark:border-slate-800 p-8 text-center bg-white dark:bg-slate-900">
          ...
          <p className="font-bold text-slate-700 dark:text-slate-200 text-sm">Không có lịch hẹn nào trong thời gian này</p>
          <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Hãy thử chọn mốc ngày khác hoặc thêm yêu cầu đặt lịch hẹn mới.</p>
        </Card>
      ) : (
          ...
          {timelineItems.map((item, idx) => {
            ...
            return (
              <div key={item.id} className="relative group">
                {/* Timeline node dot */}
                <div
                  className={`absolute -left-[23px] sm:-left-[31px] top-4 h-5 w-5 rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center shadow-sm z-10 ${
                    isDealed
                ...
                <div
                  ...
                  className={`p-4 border rounded-2xl bg-white dark:bg-slate-900 shadow-xs space-y-3 cursor-pointer transition-all hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-600 active:scale-[0.99] ${
                    isDealed
                      ? 'border-emerald-200 dark:border-emerald-800/80 bg-emerald-50/20 dark:bg-emerald-950/20'
                      : isCheckedIn
                      ? 'border-green-200 dark:border-green-800/80 bg-green-50/10 dark:bg-green-950/20'
                      : isOnTheWay
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2.5">
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-black text-base text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 px-2.5 py-0.5 rounded-lg shadow-xs">
                        ⏰ {item.time}
                      </span>
                    ...
                    <div className="flex items-center gap-1.5">
                      {!item.assigned_to ? (
                        ...
                        <Badge variant="outline" className="text-amber-700 dark:text-amber-300 border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/60 text-[10px]">
                          ⏳ Chờ xuất phát
                        </Badge>
                      )}
                    </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      <div className="font-bold text-sm text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                        ...
                        <span className="text-indigo-950 dark:text-white font-extrabold">{item.room_title || 'Căn hộ xem phòng'}</span>
                      </div>
                      {item.area && (
                    ...
                    {(item.building_address || item.notes) && (
                      <p className="text-xs text-slate-600 dark:text-slate-300 pl-5 leading-relaxed font-medium">
                        📍 {item.building_address || item.notes}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-700 dark:text-slate-200 shrink-0 text-xs">
                        {item.customer_name?.[0] || 'K'}
                      </div>
                      <div>
                        <div className="font-bold text-slate-800 dark:text-slate-100">{item.customer_name}</div>
                        <div className="font-mono text-[11px] text-slate-500 dark:text-slate-400">{item.customer_phone}</div>
                      </div>
                    </div>
                    ...
                    {!item.assigned_to ? (
                      ...
                      <Button
                        ...
                        className="bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-700 text-white font-bold h-8 text-[11px] px-3 rounded-xl gap-1"
                      >
