      ...
      try {
        ...
        if (isSingleBoardFormat) {
          ...
          for (let i = 1; i < rowsFirst.length; i++) {
            ...
            // Parse status: Ở luôn / ở ngay -> available, Đang sửa -> maintenance, date/else -> rented/soon_available
            ...
            if (soonDate) {
              ...
              if (cleanStatus.includes('sửa') || cleanStatus.includes('bảo trì') || cleanStatus.includes('trì')) {
                ...
                // Mặc định là đã cho thuê nếu ghi các từ khác như "đang ở", "đã thuê", "hết phòng"
                status = 'rented';
              }
            }
          ...
          if (wsPhong) {
            ...
            for (let i = 1; i < rows.length; i++) {
              ...
              const rawStatus = String(row[8] || 'available').trim();
              const soonDate = parseSoonDate(rawStatus);
              let status = 'available';
              let description = '';
              ...
              if (soonDate) {
                ...
                if (cleanStatus.includes('sửa') || cleanStatus.includes('bảo trì') || cleanStatus.includes('trì')) {
                  status = 'maintenance';
                } else if (cleanStatus.includes('rented')) {
                  status = 'rented';
            ...
            <CardContent className="pt-4 flex-1 flex flex-col justify-between">
              {parsing ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 text-slate-400">
                  <FileText className="h-12 w-12 opacity-30 mb-2 text-slate-600" />
