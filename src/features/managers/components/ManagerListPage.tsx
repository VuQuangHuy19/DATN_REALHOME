...
export function ManagerListPage({ isLandlordPortal = false }: ManagerListPageProps = {}) {
  ...
  // Find current landlord object if in landlord portal
  const currentLandlord = useMemo(() => {
    if (!profile) return null;
    return (
      landlordList.find(
        (l) =>
          l.id === profile.landlord_id ||
          (l as any).user_id === profile.id ||
          (l.phone && profile.phone && l.phone === profile.phone)
      ) || null
    );
  }, [landlordList, profile]);
  ...
  const managerPageSize = 6; // 6 managers per page
  ...
  // Set default landlord ID when in landlord portal
  useEffect(() => {
    if (isLandlordPortal) {
      const lid = currentLandlord?.id || profile?.landlord_id || '';
      if (lid) {
        setFilterLandlordId(lid);
        setSelectedLandlordId(lid);
      }
    }
  }, [isLandlordPortal, currentLandlord?.id, profile?.landlord_id]);
  ...
  const selectedLandlordObj = landlordList.find((l) => l.id === selectedLandlordId);
  const selectedLandlordCode = selectedLandlordObj?.code;
  const landlordBuildings = buildingList.filter((b) => {
    if (!selectedLandlordId) return false;
    return b.landlord_id === selectedLandlordId || (selectedLandlordCode && b.landlord_id === selectedLandlordCode);
  });
  ...
  const openAdd = () => {
    setEditItem(null);
    setAssignedBuildingIds([]);
    const defaultLandlordId = isLandlordPortal
      ? (currentLandlord?.id || profile?.landlord_id || filterLandlordId || '')
      : (filterLandlordId || '');
    setSelectedLandlordId(defaultLandlordId);
    setManagerAvatar('');
  ...
  const openEdit = (item: DBManager) => {
    ...
    setSelectedLandlordId(item.landlord_id || (isLandlordPortal ? currentLandlord?.id || profile?.landlord_id || '' : ''));
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <Link
            href={isLandlordPortal ? "/landlord" : "/admin"}
          ...
          >
            <ArrowLeft className="h-3.5 w-3.5" /> Quay lại {isLandlordPortal ? "trang tổng quan" : "trang trước"}
          </Link>
          <div className="flex items-center gap-2">
            ...
            <Badge className="bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300 font-mono text-xs border border-indigo-200 dark:border-indigo-800">
              {filteredManagers.length} nhân sự
            </Badge>
          </div>
          <p className="text-ink-muted text-xs sm:text-sm mt-0.5">
            {isLandlordPortal
              ? 'Danh sách nhân sự vận hành bất động sản thuộc quản lý của bạn'
              ...
              <form onSubmit={handleSave} className="p-6 space-y-5">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {isLandlordPortal ? (
                      <div className="flex h-10 w-full rounded-xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50/70 dark:bg-indigo-950/40 px-3 py-2 text-sm font-bold text-indigo-900 dark:text-indigo-200 items-center mt-1.5 gap-2">
                        <User className="h-4 w-4 text-indigo-600 shrink-0" />
                        <span className="truncate">{currentLandlord?.system_name || currentLandlord?.name || profile?.full_name || 'Chủ nhà hiện tại'}</span>
                        {currentLandlord?.code && (
                          <span className="font-mono text-xs bg-indigo-200/80 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 px-2 py-0.5 rounded-md shrink-0">
                            {currentLandlord.code}
                          </span>
                        )}
                      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className={cn(isLandlordPortal ? 'lg:col-span-12' : 'lg:col-span-8', 'space-y-3.5')}>
          {/* Controls Bar */}
          ...
          {loading ? (
            ...
            /* ── GRID CARD VIEW ───────────────────────────────────────────────── */
            <div className={cn('grid gap-4', isLandlordPortal ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 md:grid-cols-2')}>
