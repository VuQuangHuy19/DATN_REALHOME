...
import { Loader2, Sparkles, ChevronLeft, ChevronRight, Phone } from 'lucide-react';
...
export function SimilarBuildingsWidget({
  currentBuilding,
}: {
  currentBuilding: {
    id: string;
  ...
  // Group listings into BuildingGroup[] and find similar buildings
  const similarGroups = useMemo<BuildingGroup[]>(() => {
    ...
    const scored = activeGroups.map((g) => {
      let score = 0;
      if (currentBuilding.district_id && g.rooms.some((r) => r.districtId === currentBuilding.district_id)) {
        score += 5;
      }
      if (currentBuilding.area && g.area === currentBuilding.area) {
        score += 4;
      }
    ...
    return scored.map((s) => s.group).slice(0, 8);
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="text-xs bg-amber-50 text-amber-700 font-semibold border-amber-200 border px-2 py-0.5">
              {similarGroups.length} gợi ý
            </Badge>
          </div>
          <p className="text-xs text-ink-muted mt-1">
            Các tòa nhà tương đồng về khu vực, tiện ích và vị trí xung quanh (kéo ngang để xem thêm)
          </p>
        </div>

        {/* Controls */}
        {similarGroups.length > 3 && (
