...
import {
  ...
  generateLandlordCode,
  parseSoonDate,
  getCellHyperlink,
  parseLocation,
} from '../services/excel/excelParsers';
    ...
    reader.onload = (e) => {
      try {
        ...
        if (isSingleBoardFormat) {
          ...
          let managerNameColIdx = -1;
          let managerPhoneColIdx = -1;
          ...
          headerRow.forEach((cell: any, idx: number) => {
            ...
            else if (cellStr.includes('sđt chủ') || cellStr.includes('số điện thoại chủ')) landlordPhoneColIdx = idx;
            ...
            else if (cellStr.includes('số dẫn') || cellStr.includes('sđt dẫn') || cellStr.includes('sđt quản lý')) managerPhoneColIdx = idx;
            else if (cellStr.includes('người dẫn') || cellStr.includes('tên quản lý') || cellStr.includes('quản lý')) managerNameColIdx = idx;
            else if (cellStr.includes('liên hệ sales')) managerRawColIdx = idx;
          });
            ...
            if (rawBuildingAddress) {
              ...
              if (!uniqueBuildings.has(lastBuildingCode)) {
                ...
                buildings.push({
                  ...
                  manager_raw: (() => {
                    if (managerPhoneColIdx !== -1 && row[managerPhoneColIdx]) {
                      const mPhone = String(row[managerPhoneColIdx]).trim();
                      const mName = managerNameColIdx !== -1 && row[managerNameColIdx] ? String(row[managerNameColIdx]).trim() : 'Quản lý';
                      return `${mName} - ${mPhone}`;
                    }
                    if (managerRawColIdx !== -1 && row[managerRawColIdx]) {
                      return String(row[managerRawColIdx]).trim();
                    }
                    return null;
                  })()
                });
                ...
                if (bld) {
                  ...
                  if (!bld.manager_raw) {
                    if (managerPhoneColIdx !== -1 && row[managerPhoneColIdx]) {
                      const mPhone = String(row[managerPhoneColIdx]).trim();
                      const mName = managerNameColIdx !== -1 && row[managerNameColIdx] ? String(row[managerNameColIdx]).trim() : 'Quản lý';
                      bld.manager_raw = `${mName} - ${mPhone}`;
                    } else if (managerRawColIdx !== -1 && row[managerRawColIdx]) {
                      bld.manager_raw = String(row[managerRawColIdx]).trim();
                    }
                  }
                }
              }
