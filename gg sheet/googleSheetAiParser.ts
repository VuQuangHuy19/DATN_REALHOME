 ...
 * Ví dụ: "trục 01" / "01" -> "201", "trục 02" / "02" -> "202", "trục 03" -> "203", "trục 04" -> "204"
 */
 ...
 * Hệ thống tự phát hiện số tầng tối đa (ví dụ 5 tầng từ phòng 503, 504) và triển khai toàn bộ các phòng còn thiếu
 * theo từng trục từ Tầng 2 lên đến Tầng tối đa (ví dụ: 201, 301, 401, 501; 202, 302, 402, 502...).
 */
...
/**
 * Xây dựng Ghi chú chung tổng hợp cho từng Tòa nhà (tự động thay thế DVC/Internet riêng của Tòa đó nếu có)
 */
...
export function parseSheetContentProgrammatically(wb: XLSX.WorkBook): SheetImportResult | null {
  ...
  for (const sheetName of targetSheets) {
    ...
    for (let r = startRow; r < rows.length; r++) {
      ...
      // Trích xuất DVC & Internet riêng của Tòa nhà từ các dòng phòng
      ...
      // Xử lý Ghi chú riêng cho Tòa nhà (ví dụ "nhà này khách tự kéo wifi...")
      ...
      // Đảo lại nếu tiêu đề cột bị ghi ngược (ví dụ codeVal mang giá tiền)
      ...
      // Trích xuất Link Google Drive ẩn
      ...
      // Quy tắc: Nếu ô trạng thái có ghi "trống", "ở ngay", "ở luôn", "sẵn" -> available (TRỐNG).
      // Nếu ô trạng thái rỗng ("") hoặc ghi "đã ở", "đã thuê", "có khách", ngày tháng -> rented (ĐÃ Ở).
      let status = 'rented';
      if (
      ...
      const roomObj: ParsedRoom = {
        ...
        drive_media_url: roomDriveUrl || buildingDriveUrl,
      };
...
export async function parseGoogleSheetFull(sheetUrl: string): Promise<SheetImportResult> {
  ...
  // BƯỚC 1: Thử bóc tách trực tiếp bằng Programmatic Parser (Siêu nhanh 0.5s & Chính xác 100%)
  try {
    const exportXlsxUrl = `https://docs.google.com/spreadsheets/d/${sheetId}/export?format=xlsx`;
    const res = await fetch(exportXlsxUrl, {
      headers: {
...
2. BÓC TÁCH TOÀN BỘ PHÒNG (BẮT BUỘC KHÔNG ĐƯỢC BỎ SÓT BẤT KỲ PHÒNG NÀO):
   ...
   - "status": Nếu ghi "ở ngay", "ở luôn", "trống", "sẵn" -> "available". Nếu ghi "đã thuê", "có khách", hoặc rỗng -> "rented".
