...
export function GoogleSheetImportModal({
  ...
  const handleStartParse = async () => {
    ...
    try {
      ...
      setShowPreview(true);
      onOpenChange(false); // Đóng modal dán link
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg p-6 rounded-2xl border border-amber-500/30 bg-slate-950 text-white shadow-2xl">
          ...
          <div className="space-y-4 py-3">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">
                  CHỦ NHÀ PHỤ TRÁCH <span className="text-amber-400">*</span>
                </Label>
                <button
                  type="button"
              ...
              <select
                ...
                className="flex h-10 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white focus:outline-none focus:border-amber-400"
                required
              >
            ...
            <div className="space-y-2">
              <Label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                <span>ĐƯỜNG DẪN GOOGLE SHEET (SHARE LINK) <span className="text-amber-400">*</span></span>
                <span className="text-[11px] text-amber-400 font-normal">Bất kỳ ai có link đều xem được</span>
              </Label>
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-9 bg-slate-900 border-slate-700 focus:border-amber-400 text-white text-xs h-10 rounded-lg"
                />
              </div>
            </div>

            {/* Hướng dẫn bật quyền */}
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3.5 space-y-1.5 text-xs text-slate-300">
