...
export async function syncAgentKPI(companyId: string, employeeId: string, period: string) {
  try {
    ...
    const totalDepositsRent = (deposits ?? []).reduce((sum, c) => sum + (Number(c.rent_price) || 0), 0);
    const totalDepositsComm = (deposits ?? []).reduce((sum, c) => sum + (Number(c.commission_amount) || 0), 0);
    const totalDepositsCount = (deposits ?? []).length;

    const totalRentalsRent = (rentals ?? []).reduce((sum, c) => sum + (Number(c.rent_price) || 0), 0);
    const totalRentalsComm = (rentals ?? []).reduce((sum, c) => sum + (Number(c.commission_amount) || 0), 0);
